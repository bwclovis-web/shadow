...
import { markDecantSplitShipped } from "@/models/decant-split.server"
import {
...
export const POST = async (
  ...
  try {
    ...
    const split = await markDecantSplitShipped(splitId, auth.userId)
    return createSuccessResponse({ split })
  } catch (error) {
    return decantSplitRouteError(error, "Failed to mark split shipped")
  }
}
