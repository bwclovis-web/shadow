...
const ReportTraderModal = ({
  ...
  const [blockState, blockAction, isBlocking] = useActionState(
    blockUserAction,
    null as BlockUserActionState
  )

  useEffect(() => {
    if (state?.success || blockState?.success) {
      onSuccess?.()
    }
  }, [state?.success, blockState?.success, onSuccess])
    ...
    <div className="w-full p-6">
      ...
      {!state?.success && !blockState?.success && (
        <>
          ...
          <div className="mt-6 border-t border-noir-gold/20 pt-4">
            <p className="mb-1 text-sm font-medium text-noir-gold-100">{t("blockHeading")}</p>
            <p className="mb-3 text-xs text-noir-gold-100/70">{t("blockHint")}</p>
            <form action={blockAction}>
              <CSRFToken />
              <input type="hidden" name="blockedUserId" value={reportedUserId} />
              <Button
                type="submit"
                variant="secondary"
                disabled={isPending || isBlocking}
