...
export const getReviews = async (
  ...
  if (!perfumeId && !userId) throw new Error("Either perfumeId or userId is required")
...
/**
 * Fetch reviews for a specific user. Requires authentication.
 *
 ...
 */
export const getRatings = async (perfumeId: string): Promise<RatingsResponse> => {
  if (!perfumeId) throw new Error("Perfume ID is required")

  const response = await fetch(`/api/ratings?${new URLSearchParams({ perfumeId })}`)
  if (response.status === 404) throw new Error("Ratings not found")
  if (!response.ok) {
    const errorData = await response.json().catch(() => ({}))
